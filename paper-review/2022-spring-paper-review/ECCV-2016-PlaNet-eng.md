...
For anyone interested a little bit in computer science, and most specifically computer vision, a question come directly in mind: «How good a computer can be at« Gueoguesser » ? We will talk about different technique that has been used to solve this task, we will then explain and discuss the PlaNet model, which serve as  «benchmark» in the field and has been one of the first paper using deep neural network to solve the problem and then we will see different ideas that have been used to improve the accuracy.
...
Before the use of convolutional neural networks (CNN) for image processing and analysis, researchers used data driven model. One of the famous models used to solve this task in this way is (1) « IM2GPS ».
...
IM2GPS is highly dependent on the number of images available and could get some better results with more data, even close to perfect if composed of photos from every street and angle on the planet. At the moment, most of the developed technics are using large amount of data to train deep learning networks, but by focusing on image gathering and big data optimisation, there are good chances that some variant of this approach could nowadays be more competitive.
...
Based on distribution of localisation in the dataset: starting at the roots, they recursively descend the tree till each cells contained no more than a threshold number of photos and discard the regions where the number of photos is below another threshold. In the end, the size of each cell is correlated to the likelihood of a photo being taken in this area.
...
The architecture used is based one the Inception architecture. Developed by Google in 2014 is build on a succession of inception module.
...
The model consists of 74.7M parameters, which found empirically to be the optimal number. Upscaling this number wasn’t giving any improvement and reducing was affecting the accuracy.
...
The accuracy for the model has then been test on a set of 2 million pictures taken from the social media platform FlickR.
...
To improve the accuracy, the author tries to use different pictures from the same location and use a LSTM Network to extract information from multiple images. First they created for each image an embedding vector by taking the final layer before the SoftMax layer of the previously trained model. Then they feed theses vectors into an LSTM layer. They finally took the output of the last photos and feed into a SoftMax layer to performs the classification.
...
Using a group of photos with an LSTM model help to classify some images that normally have an important uncertainty. This method out performed a simple average on the single prediction of all the photos in the album.
...
One of the ideas proposes by the author is to first used a model to classify in which environment the photo has been taken and then used different models trained to geolocalise particular environment. (ISN)
...
This task has been research for a long time, and different methods have emerged. The main idea is to use polar transformation to try to approximate the satellite images, and then use some comparison metric to find the closest satellite image.
...
By detection the name of potential store or building in the image and automatically scarp  the internet for information could also be a great solution for a number of images.
...
If using the internet is not an option, this kind of information could be used as a feature extraction technics and then provide to some classification layer.
